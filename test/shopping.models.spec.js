...
describe("The Shopping Process", () => {
  ...
  describe("adding a product to a cart", () => {
    describe("product is not in a lineItem", () => {
      it("lineItem gets created", async () => {
        const yuccaCane = seed.products.yuccaCane;
        const larry = seed.users.larry;
        const cart = await larry.addToCart({ product: yuccaCane, quantity: 2 });
    ...
    describe("product is a lineItem", () => {
      it("lineItem gets updated", async () => {
        const yuccaCane = seed.products.yuccaCane;
        const larry = seed.users.larry;
        await larry.addToCart({ product: yuccaCane, quantity: 2 });
        const cart = await larry.addToCart({ product: yuccaCane, quantity: 3 });
        expect(cart.lineItems.length).to.equal(1);
  ...
  describe("decrementing a quantity in a cart", () => {
    describe("new quantity is still greater than zero", () => {
      it("lineItem gets updated", async () => {
        const yuccaCane = seed.products.yuccaCane;
        const crotonPlant = seed.products.crotonPlant;
        const larry = seed.users.larry;
        await larry.addToCart({ product: yuccaCane, quantity: 2 });
        let cart = await larry.addToCart({ product: crotonPlant, quantity: 3 });
        ...
        const lineItem = cart.lineItems.find((lineItem) => {
          return lineItem.product.name === "crotonPlant";
        });
